  ...
  return (
    <div className="relative inline-block w-80">
      <button
        ...
        className="block h-12 w-80 p-2 border-slate-800 bg-slate-900 rounded appearance-none outline-none focus:outline-none focus:ring-1 focus:ring-slate-900 focus:border-slate-900"
      >
        {selectedItem.name}
      </button>
      {isOpen && (
        <ul className="absolute min-h-12 w-80 mt-2 py-1 border rounded border-slate-800 bg-slate-900  shadow-xl">
          {items.map((item) => (
            <li
              key={item.id}
